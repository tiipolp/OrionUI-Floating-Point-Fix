...
do
    ...
    function utility:KeyPressed()
        local key = userinput.InputBegan:Wait()
        while key.UserInputType ~= Enum.UserInputType.Keyboard do
            key = userinput.InputBegan:Wait()
        end
        wait()
        return key
    end
...
do
    ...
    function library:Notify(title, text, callback)
        ...
        local close = function()
            ...
            debris:AddItem(notification, 0)
        end
        self.activeNotification = close
        notification.Accept.MouseButton1Down:Connect(function()
            if not active then 
                return
            end
            if callback then
        ...
        notification.Decline.MouseButton1Down:Connect(function()
            if not active then 
                return
            end
            if callback then
    ...
    function section:addColorPicker(title, default, callback)
        ...
        local toggle, debounce, lastColor, animate
    ...
    function section:addDropdown(title, list, callback)
        ...
        local focused
        list = list or {}
        search.Button.MouseButton1Down:Connect(function()
